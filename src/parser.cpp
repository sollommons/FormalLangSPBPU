...
//maxie:
void Parser::arithmetic_if(char RunFrom)
{
	// арифметический IF THEN ELSE FI, в коде выглядит как 
	// (⟨relation⟩)?(⟨statementList⟩|⟨expression⟩):(⟨statementList⟩|⟨expression⟩)
	// но по факту код преобразовывается прекомпилятором и выглядит так:
	// #⟨relation⟩?⟨statementList⟩|⟨expression⟩@⟨statementList⟩|⟨expression⟩%
	// выбор между ⟨statementList⟩ и ⟨expression⟩ осуществляется по параметру RunFrom
	// 'e' expression и 's' statement
	relation();
...
void Parser::statement()
{
	...
	//maxie:
		...
		if (see(T_IDENTIFIER)) {
			...
			expression();
			codegen_->emit(STORE, varAddress);
			identifierAddress_f = codegen_->getCurrentAddress();
			codegen_->emit(LOAD, varAddress);
		}
		else {
			std::ostringstream msg;
			msg << tokenToString(scanner_->token()) << " found while " <<
				tokenToString(T_IDENTIFIER) << " expected.";
			reportError(msg.str());
		}
		mustBe(T_TO);
		expression();
		...
		codegen_->emit(ADD);
		codegen_->emit(STORE, varAddress);
		codegen_->emit(JUMP, identifierAddress_f);
		codegen_->emitAt(jumpNoAddress, JUMP_NO, codegen_->getCurrentAddress());
	}
	else {
